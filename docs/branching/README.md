...
- **Phase 8:** `[partial]` (R49 ✅, R50 ✅, R51 ✅, R52 ✅, R55 ✅, R56 ✅, R58 ✅, R59 ✅, R60 ✅ implemented; R53, R54 are authored design contracts not yet implemented).
...
- [50-dual-reflection-templates.md](50-dual-reflection-templates.md): Dual-level reflection framework — fast policy-loop reflection (event-driven coherence/invariants/memory-check) plus scheduled high-level reflection (batch outcomes, regime drift, playbook updates), with deterministic tick-level validation kept separate from LLM reflection. `✅ implemented`
...
Runbooks 49–54 (and Runbook 58) translate the paper insights into implementation contracts for this
...
> exists. Runbook 58 then adds the deterministic structure engine (levels/ladders/events)
> so "support/resistance", structural targets, and reassessment triggers are first-class
> typed context instead of ad-hoc prompt interpretations. Diversified memory retrieval (51)
...
- [_prompt-budget-and-context-offload.md](_prompt-budget-and-context-offload.md): Prompt-fat reduction and token-budget runbook (ADR follow-on). Defines strategist/judge prompt budgets, multi-instrument payload shaping, and a policy to offload static/reference context to vector store or file search instead of inlining prompt prose.
